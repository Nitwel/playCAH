<template>
    <div ref="hand" class="hands" :style="{ 'paddingRight': `${-cardMargin + 10}px` }">
        <teleport
            v-for="(card, index) in hands"
            :key="card"
            :to="to[index]"
            :disabled="index in to === false || to[index] === ''"
        >
            <Card
                :id="index"
                :ref="registerCardRef"
            ...
            >
                ...
                <template #container>
                    ...
                    <transition name="scaleSelect">
                        <Button
                            v-if="placeable && blackCard && blackCard.pick in inSlot && index === Number(inSlot[blackCard.pick].id)"
                            class="accept abs"
                            icon="done"
                            rounded
                            @click="onSelect"
...
function onSelect() {
    ...
    store.state.hands = store.state.hands.filter(card => cards.includes(card) === false)
    console.log(store.state.hands, cards)
